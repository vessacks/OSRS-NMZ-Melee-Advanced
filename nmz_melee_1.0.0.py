...
zero_prayer_vision = Vision('C:\\Users\\Jeff C\\Downloads\\Code\\OpenCV files\\nmz melee\\image library\\1_hp.png', method = cv.TM_CCOEFF_NORMED, imread = cv.IMREAD_COLOR)
...
while True:
    ...
    if time.time() - last_overload_time > 300 - protect_melee_early_time or overload_off_confidence > OVERLOAD_OFF_THRESHOLD: #ie its been five minutes less the early time since last overload click or I'm seeing the overload has worn off and I haven't turned on melee in the last 10 seconds
        if time.time() - protect_melee_click_time > 10: #ie we havent been down here int eh past 10 seconds
            if protect_melee_disabled == False:
                ...
                print('protect_melee disabled because there are no more overloads. I would have turned on protect melee here')
            ...
            if time.time() - last_overload_time > 7 + protect_melee_late_time:
                if protect_melee_disabled == False:
                    ...
                    if protect_melee_on_confidence > PRAYER_THRESHOLD:
                        ...
                        print('cannot find protect_melee_on | confidence %s | giving up...' %round(protect_melee_on_confidence,3))
                        exit()
                    break
                else:
                    print('I would have turned off protect melee but it is disabled. This happens when you run out of overloads')
    ...
    if zero_prayer_confidence > ZERO_PRAYER_THRESHOLD and last_praypot_time > 5:
        print('zero prayer detected | confidence %s' %zero_prayer_confidence)
        #find praypot_ones
        screenshot = wincap.get_screenshot() 
        screenshot = cv.cvtColor(screenshot, cv.COLOR_BGR2GRAY) #grayscale
    ...
    #print('main loop complete | loop time %s' %(time.time()-loop_time))
    loop_time = time.time()
